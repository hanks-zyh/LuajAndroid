...
public class LuaThread extends Thread implements Runnable {
    ...
    public void run() {
        try {
            ...
            mLuaContext.toast(e.getMessage());
            return;
        }
        if (mIsLoop) {
            Looper.prepare();
    ...
    public void push(int what, String s) {
        if (!isRun) {
            mLuaContext.toast("thread is not running");
            return;
        }
    ...
    private void newLuaThread(String str, Object... args) {
        try {
            ...
            mLuaContext.toast(this.toString() + " " + e.getMessage());
            quit();
        }
    ...
    private void runFunc(String funcName, Object... args) {
        try {
            ...
            mLuaContext.toast(funcName + " " + e.getMessage());
        }
